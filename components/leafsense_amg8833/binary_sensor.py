...
CONF_ROI_AVAILABLE = "roi_available"
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_OVERFLOW_DETECTED): binary_sensor.binary_sensor_schema(
            device_class=DEVICE_CLASS_PROBLEM,
            entity_category=ENTITY_CATEGORY_DIAGNOSTIC,
        ),
        cv.Optional(CONF_INTERRUPT_DETECTED): binary_sensor.binary_sensor_schema(
            entity_category=ENTITY_CATEGORY_DIAGNOSTIC,
        ),
        cv.Optional(CONF_RECOVERY_ACTIVE): binary_sensor.binary_sensor_schema(
            entity_category=ENTITY_CATEGORY_DIAGNOSTIC,
        ),
        cv.Optional(CONF_ROI_AVAILABLE): binary_sensor.binary_sensor_schema(
            entity_category=ENTITY_CATEGORY_DIAGNOSTIC,
        ),
    }
)
...
async def to_code(config):
    ...
    definitions = (
        ...
        (CONF_DRIVER_PROBLEM, "set_driver_problem_binary_sensor"),
        (CONF_FRAME_AVAILABLE, "set_frame_available_binary_sensor"),
        (CONF_OVERFLOW_DETECTED, "set_overflow_detected_binary_sensor"),
        (CONF_INTERRUPT_DETECTED, "set_interrupt_detected_binary_sensor"),
        (CONF_RECOVERY_ACTIVE, "set_recovery_active_binary_sensor"),
        (CONF_ROI_AVAILABLE, "set_roi_available_binary_sensor"),
    )

    for key, setter in definitions:
        await register_optional_binary_sensor(config, parent, key, setter)
