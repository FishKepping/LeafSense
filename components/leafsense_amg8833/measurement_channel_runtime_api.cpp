...
void MeasurementChannelRuntimeApi::setup()
{
    ...
    register_service(
        ...
        std::array<std::string, 5>{"channel", "x", "y", "width", "height"});
...
void MeasurementChannelRuntimeApi::service_disable_(int channel)
{
    ...
    if (!convert_channel_(channel, index)) {
        log_result_("disable", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
...
void MeasurementChannelRuntimeApi::service_set_rectangle_(
    int channel, float x, float y, float width, float height)
{
    std::size_t index = 0U;
    if (!convert_channel_(channel, index)) {
        log_result_("set_rectangle", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
    log_result_(
        "set_rectangle",
        channel,
        controller_->setRectangle(index, {x, y, width, height}));
}

void MeasurementChannelRuntimeApi::service_polygon_begin_(int channel, int point_count)
{
    std::size_t index = 0U;
    if (!convert_channel_(channel, index)) {
        log_result_("polygon_begin", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
...
void MeasurementChannelRuntimeApi::service_polygon_point_(
    int channel, int point_index, float x, float y)
{
    std::size_t index = 0U;
    if (!convert_channel_(channel, index)) {
        log_result_("polygon_point", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
    const auto status = point_index < 0
        ? leafsense::measurement::MeasurementChannelCommandStatus::InvalidPointIndex
        : controller_->setPolygonPoint(index, static_cast<std::size_t>(point_index), {x, y});
...
void MeasurementChannelRuntimeApi::service_polygon_commit_(int channel)
{
    std::size_t index = 0U;
    if (!convert_channel_(channel, index)) {
        log_result_("polygon_commit", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
...
void MeasurementChannelRuntimeApi::service_polygon_cancel_(int channel)
{
    std::size_t index = 0U;
    if (!convert_channel_(channel, index)) {
        log_result_("polygon_cancel", channel, leafsense::measurement::MeasurementChannelCommandStatus::InvalidChannel);
        return;
    }
...
bool MeasurementChannelRuntimeApi::convert_channel_(
    int channel, std::size_t& channel_index) const
{
    if (controller_ == nullptr) {
        ESP_LOGE(TAG, "Runtime command rejected: controller is not connected");
        return false;
    }
    if (channel < kFirstPublicChannel || channel > kLastPublicChannel) return false;
...
{
    const char* text = leafsense::measurement::toString(status);
    if (status == leafsense::measurement::MeasurementChannelCommandStatus::Success ||
        ...
        ESP_LOGI(TAG, "%s channel %d: %s", command, channel, text);
    } else {
        ESP_LOGW(TAG, "%s channel %d rejected: %s", command, channel, text);
    }
}
