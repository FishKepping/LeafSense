...
LeafSenseAmg8833Component = leafsense_amg8833_ns.class_(
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_INCLUDE_INTERRUPT_MAP, default=False): cv.boolean,
...
async def to_code(config):
    ...
    cg.add(
        var.set_include_interrupt_map(
            config[CONF_INCLUDE_INTERRUPT_MAP]
        )
    )
    cg.add(
        var.set_recovery_enabled(
            config[CONF_RECOVERY_ENABLED]
        )
    )
    cg.add(
        var.set_recovery_failure_threshold(
            config[CONF_RECOVERY_FAILURE_THRESHOLD]
        )
    )
    cg.add(
